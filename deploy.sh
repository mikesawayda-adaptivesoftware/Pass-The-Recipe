...
REPO_URL="https://github.com/mikesawayda-adaptivesoftware/Pass-The-Recipe.git"
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   🍳 Pass The Recipe - Deploy Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if [[ -z $(git status -s) ]]; then
    ...
    if [ -z "$1" ]; then
        COMMIT_MSG="Update Pass-The-Recipe - $(date '+%Y-%m-%d %H:%M')"
...
docker buildx build --platform linux/amd64 -t ghcr.io/${GITHUB_USERNAME}/pass-the-recipe:latest --push .
...
echo ""
echo -e "     ${GREEN}docker pull ghcr.io/${GITHUB_USERNAME}/pass-the-recipe:latest${NC}"
echo ""
echo -e "  4. Stop/remove any old containers (if exists):"
echo ""
echo -e "     ${GREEN}docker rm -f pass-the-recipe 2>/dev/null || true${NC}"
echo ""
echo -e "  5. Run the container:"
echo ""
echo -e "     ${GREEN}docker run -d \\${NC}"
echo -e "     ${GREEN}  --name pass-the-recipe \\${NC}"
echo -e "     ${GREEN}  --restart unless-stopped \\${NC}"
echo -e "     ${GREEN}  -p 3084:3000 \\${NC}"
echo -e "     ${GREEN}  -v /mnt/user/appdata/pass-the-recipe/data:/app/data \\${NC}"
echo -e "     ${GREEN}  -v /mnt/user/appdata/pass-the-recipe/uploads:/app/uploads \\${NC}"
echo -e "     ${GREEN}  -e NODE_ENV=production \\${NC}"
echo -e "     ${GREEN}  -e PORT=3000 \\${NC}"
echo -e "     ${GREEN}  -e DB_PATH=/app/data/pass-the-recipe.db \\${NC}"
echo -e "     ${GREEN}  -e JWT_SECRET='your-secure-secret-here' \\${NC}"
echo -e "     ${GREEN}  -e CORS_ORIGIN='*' \\${NC}"
echo -e "     ${GREEN}  -e INGREDIENT_PARSER_TYPE=llm \\${NC}"
echo -e "     ${GREEN}  -e LLM_PROVIDER=openai \\${NC}"
echo -e "     ${GREEN}  -e OPENAI_API_KEY='your-openai-api-key-here' \\${NC}"
echo -e "     ${GREEN}  -e OLLAMA_MODEL='gpt-4o-mini' \\${NC}"
echo -e "     ${GREEN}  ghcr.io/${GITHUB_USERNAME}/pass-the-recipe:latest${NC}"
echo ""
echo -e "  6. Watch logs:"
echo ""
echo -e "     ${GREEN}docker logs -f pass-the-recipe${NC}"
echo ""
