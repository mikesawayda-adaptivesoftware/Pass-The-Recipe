    ...
    <div class="page-container">
      <header class="page-header">
        <div class="header-content">
          <h1>My Recipes</h1>
          <p class="subtitle">{{ recipes().length }} recipes in your collection</p>
        </div>
...
export class RecipeListComponent implements OnInit, OnDestroy {
  ...
  private readonly SCROLL_KEY = 'recipe-list';
  ...
  async ngOnInit(): Promise<void> {
    await Promise.all([this.loadRecipes(), this.loadFavorites()]);
  ...
  async loadRecipes(): Promise<void> {
    ...
    try {
      ...
      this.snackBar.open('Failed to load recipes', 'Dismiss', { duration: 3000 });
    } finally {
  ...
  async loadFavorites(): Promise<void> {
    try {
      const favorites = await this.recipeService.getFavorites();
      const ids = new Set(favorites.map(r => r.id!).filter(Boolean));
  ...
  applyFilters(): void {
    ...
    // Apply ingredient filter (AND logic - must have ALL selected ingredients)
      ...
      filtered = filtered.filter(recipe => {
        return this.selectedIngredients().every(selectedIng => {
          ...
          return recipe.ingredients?.some(recipeIng => {
            ...
            // Check for partial match (e.g., "beef" matches "ground beef")
